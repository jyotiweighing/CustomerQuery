...
//     const payload = {
//       ...data,
//       softwareFeature: Array.isArray(data.softwareFeature) ? data.softwareFeature : [],
//       material: Array.isArray(data.material)
//         ? data.material.map((m) => typeof m === "string" ? { name: m, quantity: 1 } : { name: m.name, quantity: Math.max(1, Number(m.quantity) || 1) })
//         : [],
//     };
...
const ITEMS_PER_PAGE = 10;
  ...
  // If the dashboard opens /installation?taskId=TSK-xxxx, find the
  // corresponding installation and open its details modal automatically.
  useEffect(() => {
    if (!taskIdFromUrl || !tasks.length || !installations.length) return;

    const targetTask = tasks.find((task) =>
      [task.taskId, task.taskCode, task._id]
        .filter(Boolean)
        .map(String)
        .includes(String(taskIdFromUrl))
    );

    if (!targetTask) return;

    const installationId =
      targetTask.installationId ||
      targetTask.installation?._id ||
      targetTask.installation?._id;

    if (!installationId) return;

    const targetInstallation = installations.find(
      (installation) => String(installation._id) === String(installationId)
    );

    if (targetInstallation) {
      setSelectedDetails(targetInstallation);
    }
  }, [taskIdFromUrl, tasks, installations]);
  ...
  const toggleMaterial = (material) => {
    setFormData((prev) => {
      const current = Array.isArray(prev.material) ? prev.material : [];
      const exists = current.some((item) =>
        typeof item === "string" ? item === material : item?.name === material
      );
      return {
        ...prev,
        material: exists
          ...
          : [...current.map((item) => typeof item === "string" ? { name: item, quantity: 1 } : item), { name: material, quantity: 1 }],
      };
    });
  };

  const setMaterialQuantity = (material, quantity) => {
    const qty = Math.max(1, Number(quantity) || 1);
    setFormData((prev) => ({
      ...prev,
      material: (prev.material || []).map((item) =>
        (typeof item === "string" ? item : item?.name) === material
          ? { name: material, quantity: qty }
  ...
  const toggleSoftwareFeature = (feature) => {
    setFormData((prev) => ({
      ...prev,
      softwareFeature: (prev.softwareFeature || []).includes(feature)
        ? prev.softwareFeature.filter((item) => item !== feature)
        : [...(prev.softwareFeature || []), feature],
    }));
  };
    ...
    const payload = {
      ...
      material: Array.isArray(data.material)
        ? data.material.map((m) =>
            typeof m === "string"
              ? { name: m, quantity: 1 }
              : { name: m.name, quantity: Math.max(1, Number(m.quantity) || 1) }
          )
        : [],
    };
    ["billNumber", "billDate", "installationDate", "salesPersonName"].forEach(
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    >
                      <option value="Standard Software">Standard Software</option>
                      <option value="Photo Capturing">Photo Capturing</option>
                      <option value="Master Slave">Master Slave</option>
                      <option value="Unmaned Software">Unmaned Software</option>
                    </select>
                  </div>
                  <div className="md:col-span-3">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 rounded-xl border border-slate-200/80 bg-white/60 p-3">
                      {SOFTWARE_FEATURES.map((feature) => (
                        <label key={feature} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-[11px] font-semibold text-slate-700 cursor-pointer hover:border-blue-300">
                          <input
                            type="checkbox"
                            checked={(formData.softwareFeature || []).includes(feature)}
                  ...
                  <div className="md:col-span-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 rounded-xl border border-slate-200/80 bg-white/60 p-3">
                      {MATERIAL_OPTIONS.map((material) => {
                        const selected = (formData.material || []).find(
                          (item) => (typeof item === "string" ? item : item?.name) === material
                        );
                        return (
                          <div key={material} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-2.5 py-2">
                            <input
                              type="checkbox"
                              checked={!!selected}
                              ...
                              className="accent-blue-600"
                            />
                            <span className="flex-1 text-[11px] font-semibold text-slate-700">{material}</span>
                            {selected && (
                              <input
                                type="number"
                                min="1"
                                value={selected.quantity || 1}
                                onChange={(e) => setMaterialQuantity(material, e.target.value)}
                                className="w-16 rounded-md border border-slate-300 px-2 py-1 text-center text-[11px] font-bold"
                              />
                            )}
                          </div>
                        );
...
function buildEditState(item) {
  return {
    ...
    softwareType: item?.softwareType || "Standard Software",
    softwareFeature: Array.isArray(item?.softwareFeature) ? item.softwareFeature : [],
    material: Array.isArray(item?.material)
      ? item.material.map((m) => typeof m === "string" ? { name: m, quantity: 1 } : { name: m?.name, quantity: Number(m?.quantity) || 1 })
      : [],
...
function DetailModal({
  ...
  const toggleEditSoftwareFeature = (feature) => {
    setEditData((prev) => ({
      ...prev,
      softwareFeature: (prev.softwareFeature || []).includes(feature)
        ? prev.softwareFeature.filter((item) => item !== feature)
  ...
  const toggleEditMaterial = (material) => {
    setEditData((prev) => {
      const current = (prev.material || []).map((item) =>
        typeof item === "string" ? { name: item, quantity: 1 } : item
      );
      const exists = current.some((item) => item?.name === material);
      return {
        ...prev,
        material: exists
          ? current.filter((item) => item?.name !== material)
          : [...current, { name: material, quantity: 1 }],
      };
    });
  };

  const setEditMaterialQuantity = (material, quantity) => {
    const qty = Math.max(1, Number(quantity) || 1);
    setEditData((prev) => ({
      ...prev,
      material: (prev.material || []).map((item) =>
        item?.name === material ? { ...item, quantity: qty } : item
      ),
    }));
  };
      ...
      <div className="bg-slate-50/95 backdrop-blur-2xl rounded-3xl shadow-2xl w-full max-w-4xl border border-white/80 relative overflow-hidden flex flex-col">
        ...
        <div className="p-4 sm:p-5 space-y-3.5 no-scrollbar max-h-[70vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
            ...
            {/* Software Features */}
            <div className="bg-white/80 rounded-2xl p-3.5 border border-slate-200/80 shadow-sm mb-3">
              <div className="flex items-center gap-1.5 text-indigo-600 mb-2 font-bold text-[10px] uppercase tracking-wider">
              ...
              {isEditing ? (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {SOFTWARE_FEATURES.map((feature) => (
                    <label key={feature} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[10px] font-semibold text-slate-700 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={(editData.softwareFeature || []).includes(feature)}
                        onChange={() => toggleEditSoftwareFeature(feature)}
                        className="accent-blue-600"
                      />
                      <span>{feature}</span>
                    </label>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(item.softwareFeature || []).length
                    ? item.softwareFeature.map((feature) => (
                        <span key={feature} className="rounded-full bg-violet-50 border border-violet-200 px-2.5 py-1 text-[10px] font-bold text-violet-700">{feature}</span>
                      ))
                    : <span className="text-xs font-semibold text-slate-400">No software feature selected</span>}
                </div>
              )}
            </div>
            ...
            <div className="bg-white/80 rounded-2xl p-3.5 border border-slate-200/80 shadow-sm mb-3">
              ...
              {isEditing ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {MATERIAL_OPTIONS.map((material) => {
                    const selected = (editData.material || []).find((item) => item?.name === material);
                    return (
                      <div key={material} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[10px] font-semibold text-slate-700">
                        <input
                          type="checkbox"
                          checked={!!selected}
                          onChange={() => toggleEditMaterial(material)}
                          className="accent-blue-600"
                        />
                        ...
                        {selected && (
                          <input
                            ...
                            min="1"
                            value={selected.quantity || 1}
                            onChange={(e) => setEditMaterialQuantity(material, e.target.value)}
                            className="w-14 rounded border border-slate-300 px-1.5 py-1 text-center text-[10px] font-bold"
                          />
                        )}
                      </div>
                    );
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(item.material || []).length ? item.material.map((material, index) => {
                    const m = typeof material === "string" ? { name: material, quantity: 1 } : material;
                    return (
                      <span key={`${m.name}-${index}`} className="rounded-full bg-blue-50 border border-blue-200 px-2.5 py-1 text-[10px] font-bold text-blue-700">
                        {m.name} × {m.quantity || 1}
                      </span>
                    );
                  }) : <span className="text-xs font-semibold text-slate-400">No material selected</span>}
                </div>
              )}
            </div>
