        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur-xl dark:border-slate-800/70 dark:bg-slate-900/80">
            ...
            <div className="mt-2 border-t border-slate-100 pt-3 dark:border-slate-800">
              <p className="mb-2 text-xs font-semibold text-slate-400">Software Features</p>
              <div className="flex flex-wrap gap-2">
                {(task.softwareFeature || []).length
                  ? task.softwareFeature.map((feature) => (
                      <span key={feature} className="rounded-full border border-violet-200 bg-violet-50 px-2.5 py-1 text-[10px] font-bold text-violet-700 dark:border-violet-500/30 dark:bg-violet-500/10 dark:text-violet-300">{feature}</span>
                    ))
                  : <span className="text-sm font-bold text-slate-400">—</span>}
              </div>
            </div>
            <div className="mt-2 border-t border-slate-100 pt-3 dark:border-slate-800">
              <p className="mb-2 text-xs font-semibold text-slate-400">Material & Quantity</p>
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                {(task.material || []).length ? task.material.map((item, index) => {
                  const material = typeof item === 'string' ? { name: item, quantity: 1 } : item;
                  return (
                    <div key={`${material.name}-${index}`} className="flex items-center justify-between rounded-xl border border-blue-100 bg-blue-50/70 px-3 py-2 dark:border-blue-500/20 dark:bg-blue-500/10">
                      <span className="text-[11px] font-bold text-blue-700 dark:text-blue-300">{material.name}</span>
                      <span className="rounded-lg bg-white px-2 py-1 text-[10px] font-black text-slate-700 shadow-sm dark:bg-slate-800 dark:text-slate-200">Qty: {material.quantity || 1}</span>
                    </div>
                  );
                }) : <span className="text-sm font-bold text-slate-400">—</span>}
              </div>
